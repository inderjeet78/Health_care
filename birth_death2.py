...
# -*- coding: utf-8 -*-
"""
Created on Mon Apr 16 19:17:25 2018
...
for i in range(366):
    if i==0:
        ...
        rec_cost[i] = ((RES[i,2]-RES[i-1,2])*P*(RES[i,0]+RES[i,1]+RES[i,2])*recc)
...
for spine in ('top', 'right', 'bottom', 'left'):
    ax.spines[spine].set_visible(False)
plt.savefig('bd213.png', dpi=900)
plt.show()
print ("Total Cost spent on Treatement in 1 year :",sum(rec_cost))
pd.DataFrame({'Days': time, 'Susceptible': RES[:,0], 'Infected': RES[:,1], 'Recovered': RES[:,2]}).to_csv("birth_death21.csv", index=False)
...
pl.subplot(311)
pl.plot(RES[:,0], '-y', label='Susceptibles')
pl.title('SIR Model with Birth and Death rate cosidered and beta = 2 x gamma')
pl.xlabel('Time')
pl.ylabel('Susceptibles')
pl.subplot(312)
pl.plot(RES[:,1], '-r', label='Infectious')
...
for spine in ('top', 'right', 'bottom', 'left'):
    ax.spines[spine].set_visible(False)
plt.savefig('bd223.png', dpi=900)
plt.show()
print ("Total Cost spent on Treatement in 1 year :",sum(rec_cost))
pd.DataFrame({'Days': time, 'Susceptible': RES[:,0], 'Infected': RES[:,1], 'Recovered': RES[:,2]}).to_csv("birth_death23.csv", index=False)
...
for spine in ('top', 'right', 'bottom', 'left'):
    ax.spines[spine].set_visible(False)
plt.savefig('bd233.png', dpi=900)
plt.show()
print ("Total Cost spent on Treatement in 1 year :",sum(rec_cost))
pd.DataFrame({'Days': time, 'Susceptible': RES[:,0], 'Infected': RES[:,1], 'Recovered': RES[:,2]}).to_csv("birth_death24.csv", index=False)
